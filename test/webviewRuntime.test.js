...
test("webview exposes a controllable forever run loop", () => {
  ...
  assert.match(source, /\.tick-counter\s*\{[^}]*align-items: center;[^}]*height: 28px;[^}]*padding: 0 8px;/);
  assert.match(source, /\.tick-counter > \*\s*\{[^}]*height: 16px;[^}]*line-height: 16px;/);
  assert.match(source, /if \(openNativeButton\.disabled\) return;/);
  assert.match(source, /openNativeButton\.setAttribute\("aria-busy", String\(opening\)\)/);
  assert.match(source, /finally \{\s*await webviewPanel\.webview\.postMessage\(\{ type: "native-opening", opening: false \}\)/);
...
test("webview mounts a local Three.js 3D view when runtime state is available", () => {
  ...
  assert.match(source, /function addThreePatches\(scene, THREE, viewState, pickables, opaqueOnly = false\)/);
  ...
  assert.match(source, /function addThreeTurtles\(scene, THREE, turtles, pickables, opaqueOnly = false\)/);
  assert.match(source, /function addThreeLineTurtleGroup\(scene, THREE, group, pickables, active\)/);
