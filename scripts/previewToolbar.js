...
function page(source) {
  ...
  const setStatus = source.slice(source.indexOf("function setStatus("), source.indexOf("function updateEditorLayout()"));
...
const messages=[],vscode={postMessage:message=>messages.push(message)};
...
document.querySelector('#check').onclick=()=>{
 ...
 for(const font of [11,13,16]) for(const zoom of [0.8,1,1.25]) for(const status of ['Ready','Updated after iterate','Running go','Stopping','A long status with a horizontal scrollbar '.repeat(5)]){
  document.documentElement.style.setProperty('--vscode-font-size',font+'px');document.body.style.zoom=zoom;setStatus(status);
 ...
 document.documentElement.style.setProperty('--vscode-font-size','13px');document.body.style.zoom=1;state.runLoop=null;updateRunControls();setStatus('Ready');
 document.querySelector('#results').textContent=JSON.stringify({cases:checks.length,inlineStatusRemoved:!document.querySelector('#status'),outputMessages:messages.length,unstable:checks.filter(c=>!c.stable),offCenter:checks.filter(c=>!c.centered),checks},null,2);
};
</script></html>`;
}
