...
import { createViewMouseInput, isViewMouseState, type ViewMouseState } from "./viewMouse";
...
interface WebviewEditorStatusMessage {
  readonly type: "editor-status";
  readonly message: string;
}

interface WebviewInfoLinkMessage {
  readonly type: "info-link";
  readonly href: string;
}

interface WebviewInfoImageMessage {
  readonly type: "info-image";
  readonly href: string;
...
interface WebviewMouseMessage {
  readonly type: "view-mouse";
  readonly state: ViewMouseState;
}
...
type WebviewMessage =
  ...
  | WebviewEditorStatusMessage
  | WebviewInfoLinkMessage
  | WebviewInfoImageMessage
...
export class NetLogoModelEditorProvider implements vscode.CustomTextEditorProvider {
  ...
  public static register(context: vscode.ExtensionContext, runner: NetLogoRunner, output: vscode.OutputChannel): vscode.Disposable {
    return vscode.window.registerCustomEditorProvider(
      NetLogoModelEditorProvider.viewType,
      new NetLogoModelEditorProvider(context, runner, output),
      {
  ...
  public async resolveCustomTextEditor(
    ...
    webviewPanel.webview.onDidReceiveMessage(async (message: WebviewMessage) => {
      if (message.type === "view-mouse") {
        if (webviewPanel.visible && isViewMouseState(message.state)) {
      ...
      if (message.type === "info-image") {
        if (typeof message.requestId !== "string" || typeof message.href !== "string") return;
        try {
          const data = await loadInfoLocalImage(document.fileName, message.href);
      ...
      if (message.type === "info-link") {
        ...
        try {
          const external = externalInfoUrl(message.href);
          if (external) {
            if (!await vscode.env.openExternal(vscode.Uri.parse(external))) throw new Error("The link could not be opened.");
          } else {
            const file = await resolveInfoLocalFile(document.fileName, message.href);
...
<body>
  ...
  <script nonce="${nonce}">
    ...
    const createViewMouseInput = ${createViewMouseInput.toString()};
