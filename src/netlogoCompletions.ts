...
import { analyzeNetLogoLocalCompletions, type LocalCompletion } from "./modelCompletions";
import { parseNetLogoModel } from "./modelFormat";

interface NetLogoCompletionSpec {
...
const snippetCompletions: readonly NetLogoCompletionSpec[] = [
  {
    label: "to procedure",
...
export function createNetLogoCompletionProvider(): vscode.CompletionItemProvider {
  const items = [
...
function completionItemFromSpec(spec: NetLogoCompletionSpec, sortText: string): vscode.CompletionItem {
...
function localCompletionItems(document: vscode.TextDocument): vscode.CompletionItem[] {
  const model = parseNetLogoModel(document.getText(), document.fileName);
  return analyzeNetLogoLocalCompletions(model.code).map((completion, index) => localCompletionItem(completion, index));
}

function localCompletionItem(completion: LocalCompletion, index: number): vscode.CompletionItem {
...
function completionItemKind(completion: LocalCompletion): vscode.CompletionItemKind {
...
function isInsideCommentOrString(line: string, character: number): boolean {
  let inString = false;
