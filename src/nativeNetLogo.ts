...
export interface NativeNetLogoInstance {
  readonly pid: number;
  readonly appPath: string;
  readonly startedAt: number;
}

export interface NativeLaunchRecord extends NativeNetLogoInstance {
  // This is the file supplied at launch, not a live document query. Native
  // File > Open / Save As can change the model without replacing the process.
  readonly modelPath: string;
}

export interface NativeLaunchStorage {
  read(): unknown;
  write(records: readonly NativeLaunchRecord[]): Promise<void>;
}

export interface NativeDesktop {
  list(): Promise<readonly NativeNetLogoInstance[]>;
  launch(appPath: string, modelPath: string): Promise<NativeNetLogoInstance>;
  activate(instance: NativeNetLogoInstance): Promise<void>;
}

export type NativeOpenResult =
  | { readonly action: "opened" | "activated"; readonly pid: number }
  | { readonly action: "untracked"; readonly instances: readonly NativeNetLogoInstance[] };
...
export async function listMacNetLogoInstances(): Promise<readonly NativeNetLogoInstance[]> {
  const value: unknown = JSON.parse(await runCommand("/usr/bin/osascript", ["-l", "JavaScript", "-e", listMacInstancesScript]));
  ...
  return value as NativeNetLogoInstance[];
}

export async function launchMacNetLogoModel(appPath: string, modelPath: string): Promise<NativeNetLogoInstance> {
  const value: unknown = JSON.parse(await runCommand("/usr/bin/osascript", ["-l", "JavaScript", "-e",
...
const macDesktop: NativeDesktop = {
...
export class NativeNetLogoLauncher {
  private readonly pending = new Map<string, Promise<NativeOpenResult>>();
  private queue: Promise<void> = Promise.resolve();
  private records: readonly NativeLaunchRecord[] = [];

  constructor(
    private readonly storage?: NativeLaunchStorage,
    private readonly desktop: NativeDesktop = macDesktop
  ) {}

  async openMacModel(appPath: string, filePath: string): Promise<NativeOpenResult> {
  ...
  private async openOrActivate(appPath: string, modelPath: string): Promise<NativeOpenResult> {
    ...
    if (live.length !== saved.length) await this.saveRecords(live);
  ...
  async activateExisting(instance: NativeNetLogoInstance): Promise<void> {
    ...
    if (!current) throw new Error("That NetLogo session has closed. Try again; no new copy was opened.");
    await this.desktop.activate(current);
  }

  private async saveRecords(records: readonly NativeLaunchRecord[]): Promise<void> {
    this.records = records;
    if (this.storage) await this.storage.write(records);
  }
}

function sameInstance(left: NativeNetLogoInstance, right: NativeNetLogoInstance): boolean {
  return left.pid === right.pid && left.startedAt === right.startedAt && left.appPath === right.appPath;
}

function isNativeInstance(value: unknown): value is NativeNetLogoInstance {
...
function readRecords(value: unknown): readonly NativeLaunchRecord[] {
  ...
  return value as NativeLaunchRecord[];
}

function runCommand(command: string, args: readonly string[]): Promise<string> {
  return new Promise((resolve, reject) => {
    execFile(command, args, { timeout: 15000, maxBuffer: 1024 * 1024, encoding: "utf8" }, (error, stdout, stderr) => {
