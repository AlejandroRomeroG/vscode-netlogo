import type { ParsedPlotCsv, PlotCsvPen, PlotCsvPoint } from "./plotCsv";

/** Decode a complete native plot without a CSV conversion or loss of RGB colors. */
export function parsePlotBinary(data: Buffer): ParsedPlotCsv {
  let offset = 0;
  ...
  const count = (minimumRecordBytes: number): number => {
    const value = integer();
